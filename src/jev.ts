/**
 * Zero-dependency Jev client. Ported from jev-lint
 * (github.com/mizchi/jev-lint, MIT).
 *
 ...
 * the question set, which keeps the estimator in `estimate.ts` free to be
 * approximate.
 */
import { estimateTokens, usdFor } from "./estimate.ts";
...
export class Pacer {
  ...
  constructor(
    rate = envNumber("JEV_LEXER_TOKENS_PER_SECOND") ?? DEFAULT_TOKENS_PER_SECOND,
    burst = envNumber("JEV_LEXER_TOKEN_BURST") ?? DEFAULT_TOKEN_BURST,
    now = Date.now(),
  ) {
    this.rate = rate;
    this.burst = burst;
...
export class Jev implements AskClient {
  ...
  constructor({
    ...
    this.model = model ?? process.env.JEV_LEXER_MODEL ?? DEFAULT_MODEL;
  ...
  get usd(): number {
    return usdFor(this.inputTokens);
  }
